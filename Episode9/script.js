...
inputTag.addEventListener("keyup", (event) => {
    ...
    filteredProduct = products.filter(key => {
      return key.title.toLowerCase().includes(searchText);  
    });
    console.log("filtered product is ", filteredProduct);
    ...
    if(hasProductToShow) {
        for (let i = 0; i < filteredProduct.length; i++) {
            ...
            productItemContainerTag.id = filteredProduct[i].id;
...
const navigateAndSelectProduct = (key) => {
    if (key === "ArrowDown") {
        ...
        indexToSelect += 1;
        const productItemToSelect = selectProduct(indexToSelect);
        if(indexToSelect > 0){
            deselectProduct();
        }
        productItemToSelect.classList.add("selected");
    }
    else if (key === "ArrowUp") {
        if (indexToSelect === -1) {
            return;
...
const selectProduct = (index) => {
    ...
    const productItemToSelect = document.getElementById(productToSelect);
    productItemToSelect.style.backgroundColor = "#124BFF";
    productItemToSelect.firstChild.style.color = "white";
    return productItemToSelect;
}
